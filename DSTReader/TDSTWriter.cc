...
//  if(!VERTStore&&BankStatus("VERT")>0) VERTStore = new TClonesArray("TVERTClass",1);
  ...
  if(BankStatus("SCRC")>0) SCRCStore = new TClonesArray("TSCRCClass",1);
  if(BankStatus("HBLA")>0) HBLAStore = new TClonesArray("THBLAClass",1);
...
//   nPARTStore = 0;
   ...
   if(BankStatus("SCRC")>0) if(SCRCStore) SCRCStore->Clear();
   if(BankStatus("HBLA")>0) if(HBLAStore) HBLAStore->Clear();
...
//  return;
  ...
  if(BankStatus("SCRC")>0) SCRCBranch = fEventTree->Branch("SCRC",&SCRCStore,bufsize,split);
  if(BankStatus("HBLA")>0) HBLABranch = fEventTree->Branch("HBLA",&HBLAStore,bufsize,split);
...
void   TDSTWriter::CopyBanks(TVirtualReader *fReader)
{
  ...
  //
  //  // ----- Fill SCRC Buffer from the Reader --------
  //    
  if(BankStatus("SCRC")>0){
    int  n_SCRC_rows = fReader->GetNRows("SCRC");
    for(int i=0;i<n_SCRC_rows;i++){
      TObject *_ptr = fReader->GetBankRow("SCRC",i);
      AddBank("SCRC",_ptr);
    }
  }
  ...
  //
  //  //  // ----- Fill HBLA Buffer from the Reader --------
  //    //    
  if(BankStatus("HBLA")>0){
    int  n_HBLA_rows = fReader->GetNRows("HBLA");
    for(int i=0;i<n_HBLA_rows;i++){
      TObject *_ptr = fReader->GetBankRow("HBLA",i);
      AddBank("HBLA",_ptr);
    }
  }
...
//	printf("Adding SCPB Bank\n");
  ...
  if(fBnkName.CompareTo("SCRC")==0){
//	printf("Adding SCRC Bank\n");
    TSCRCClass  *SCRC_p = static_cast<TSCRCClass*> (_bnkptr);
    TClonesArray &tSCRCbank = *SCRCStore;
    new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
  }


  if(fBnkName.CompareTo("HBLA")==0){
//      printf("Adding HBLA Bank\n");
     THBLAClass  *HBLA_p = static_cast<THBLAClass*> (_bnkptr);
