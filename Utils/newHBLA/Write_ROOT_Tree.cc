...
TClonesArray *HBLAStore;
static TClonesArray *fgHBLAStore;
...
//=========================================== SCRC Banks =========================
      //
       ...
       if(kSCRC1.get_nrows() > 0){
	 for(int j = 0; j < kSCRC1.get_nrows(); j++){
	   Fill_SCRC_Bank(SCRC_p, &kSCRC1, j,1);
       ...
       SCRC    kSCRC2(2);
          SCRC_p = new TSCRCClass();
       if(kSCRC2.get_nrows() > 0){
         for(int j = 0; j < kSCRC2.get_nrows(); j++){
           Fill_SCRC_Bank(SCRC_p, &kSCRC2, j,2);
           TClonesArray &tSCRCbank = *SCRCStore;
           new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
         }
       }
       ...
       SCRC    kSCRC3(3);
          SCRC_p = new TSCRCClass();
       if(kSCRC3.get_nrows() > 0){
         for(int j = 0; j < kSCRC3.get_nrows(); j++){
           Fill_SCRC_Bank(SCRC_p, &kSCRC3, j,3);
           TClonesArray &tSCRCbank = *SCRCStore;
           new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
         }
       }
       ...
       SCRC    kSCRC4(4);
          SCRC_p = new TSCRCClass();
       if(kSCRC4.get_nrows() > 0){
         for(int j = 0; j < kSCRC4.get_nrows(); j++){
           Fill_SCRC_Bank(SCRC_p, &kSCRC4, j,4);
           TClonesArray &tSCRCbank = *SCRCStore;
           new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
         }
       }
       ...
       SCRC    kSCRC5(5);
       SCRC_p = new TSCRCClass();
       if(kSCRC5.get_nrows() > 0){
         for(int j = 0; j < kSCRC5.get_nrows(); j++){
           Fill_SCRC_Bank(SCRC_p, &kSCRC5, j,5);
           TClonesArray &tSCRCbank = *SCRCStore;
           new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
         }
       }
       ...
       SCRC    kSCRC6(6);
       SCRC_p = new TSCRCClass();
       if(kSCRC6.get_nrows() > 0){
         for(int j = 0; j < kSCRC6.get_nrows(); j++){
           Fill_SCRC_Bank(SCRC_p, &kSCRC6, j,6);
           TClonesArray &tSCRCbank = *SCRCStore;
           new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
         }
       }
...
// ==================    HBLA Bank =============================
      ...
      HBLA    kHBLA1(1);
       THBLAClass   *HBLA_p = new THBLAClass();
       if(kHBLA1.get_nrows() > 0){
         for(int j = 0; j < kHBLA1.get_nrows(); j++){
           Fill_HBLA_Bank(HBLA_p, &kHBLA1, j,1);
           TClonesArray &tHBLAbank = *HBLAStore; 
           new(tHBLAbank[nHBLA_Store++]) THBLAClass(HBLA_p);
         }
       }
       ...
       HBLA    kHBLA2(2);
          HBLA_p = new TSCHBLAass();
       if(kHBLA2.get_nrows() > 0){
         for(int j = 0; j < kHBLA2.get_nrows(); j++){
           Fill_HBLA_Bank(HBLA_p, &kHBLA2, j,2);
           TClonesArray &tHBLAbank = *HBLAStore;
           new(tHBLAbank[nHBLA_Store++]) THBLAClass(HBLA_p);
         }
       }
       ...
       HBLA    kHBLA3(3);
          HBLA_p = new THBLAClass();
       if(kHBLA3.get_nrows() > 0){
         for(int j = 0; j < kHBLA3.get_nrows(); j++){
           Fill_HBLA_Bank(HBLA_p, &kHBLA3, j,3);
           TClonesArray &tHBLAbank = *HBLAStore;
           new(tHBLAbank[nHBLA_Store++]) THBLAClass(HBLA_p);
         }
       }
      ...
      HBLA    kHBLA4(4);
       THBLAClass   *HBLA_p = new THBLAClass();
       if(kHBLA4.get_nrows() > 0){
         for(int j = 0; j < kHBLA4.get_nrows(); j++){
           Fill_HBLA_Bank(HBLA_p, &kHBLA4, j,1);
           TClonesArray &tHBLAbank = *HBLAStore; 
           new(tHBLAbank[nHBLA_Store++]) THBLAClass(HBLA_p);
         }
       }
       ...
       HBLA    kHBLA5(5);
          HBLA_p = new TSCHBLAass();
       if(kHBLA5.get_nrows() > 0){
         for(int j = 0; j < kHBLA5.get_nrows(); j++){
           Fill_HBLA_Bank(HBLA_p, &kHBLA5, j,2);
           TClonesArray &tHBLAbank = *HBLAStore;
           new(tHBLAbank[nHBLA_Store++]) THBLAClass(HBLA_p);
         }
       }
       ...
       HBLA    kHBLA6(6);
          HBLA_p = new THBLAClass();
       if(kHBLA6.get_nrows() > 0){
         for(int j = 0; j < kHBLA6.get_nrows(); j++){
           Fill_HBLA_Bank(HBLA_p, &kHBLA6, j,3);
           TClonesArray &tHBLAbank = *HBLAStore;
           new(tHBLAbank[nHBLA_Store++]) THBLAClass(HBLA_p);
         }
       }
...
//==================================== DC0 Banks ===================================
     //
       ...
       if(kDC01.get_nrows() > 0){
	 for(int j = 0; j < kDC01.get_nrows(); j++){
	   Fill_DC0_Bank(DC0_p, &kDC01, j);
       ...
       DC0    kDC02(2);
       DC0_p = new TDC0Class();
       if(kDC02.get_nrows() > 0){
         for(int j = 0; j < kDC02.get_nrows(); j++){
           Fill_DC0_Bank(DC0_p, &kDC02, j);
           TClonesArray &tDC0bank = *DC0Store;
           new(tDC0bank[nDC0_Store++]) TDC0Class(DC0_p);
         }
       }
       ...
       DC0    kDC03(3);
       DC0_p = new TDC0Class();
       if(kDC03.get_nrows() > 0){
         for(int j = 0; j < kDC03.get_nrows(); j++){
           Fill_DC0_Bank(DC0_p, &kDC03, j);
           TClonesArray &tDC0bank = *DC0Store;
           new(tDC0bank[nDC0_Store++]) TDC0Class(DC0_p);
         }
       }
       ...
       DC0    kDC04(4);
       DC0_p = new TDC0Class();
       if(kDC04.get_nrows() > 0){
         for(int j = 0; j < kDC04.get_nrows(); j++){
           Fill_DC0_Bank(DC0_p, &kDC04, j);
           TClonesArray &tDC0bank = *DC0Store;
           new(tDC0bank[nDC0_Store++]) TDC0Class(DC0_p);
         }
       }
       ...
       DC0    kDC05(5);
       DC0_p = new TDC0Class();
       if(kDC05.get_nrows() > 0){
         for(int j = 0; j < kDC05.get_nrows(); j++){
           Fill_DC0_Bank(DC0_p, &kDC05, j);
           TClonesArray &tDC0bank = *DC0Store;
           new(tDC0bank[nDC0_Store++]) TDC0Class(DC0_p);
         }
       }
       ...
       DC0    kDC06(6);
       DC0_p = new TDC0Class();
       if(kDC06.get_nrows() > 0){
         for(int j = 0; j < kDC06.get_nrows(); j++){
           Fill_DC0_Bank(DC0_p, &kDC06, j);
           TClonesArray &tDC0bank = *DC0Store;
           new(tDC0bank[nDC0_Store++]) TDC0Class(DC0_p);
         }
       }
...
void Initialize_Branches(TTree *tree,TTree *scaler_tree){
  ...
  HBLABranch = tree->Branch("HBLA",&fgHBLAStore,bufsize,split);
...
void Init_Clones()
{
  ...
  if(!fgHBLAStore) fgHBLAStore = new TClonesArray("THBLAClass",1);
  HBLAStore  = fgHBLAStore;
...
void Clear_Clones()
{
  ...
  if(fgHBLAStore) HBLAStore->Clear();
